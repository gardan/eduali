...
namespace Ilc.Data.Helpers
{
    ...
    /// LMP creates a new provider per client request.
    /// </remarks>
    public class RepositoryProvider : IRepositoryProvider
    {
        /// <summary>
        /// The <see cref="RepositoriesFactories"/> with wich to create a new repository.
        /// </summary>
        /// <remarks>
        /// Should be initialized by constructor injection.
        /// </remarks>
        private RepositoriesFactories _repositoriesFactories;

        /// <summary>
        /// Get the dictionary of repository objects, keyed by repository type.
        /// </summary>
        /// <remarks>
        /// Caller must know how to cast the repository object to a usefull type.
        ...
        public RepositoryProvider(RepositoriesFactories repositoriesFactories)
        {
            _repositoriesFactories = repositoriesFactories;
            Repositories = new Dictionary<Type, object>();
        }

        /// <summary>
        /// Get and set the <see cref="DbContext"/> with wich to initialize a repository
        /// if one must be created.
        /// </summary>
        public DbContext DbContext { get; set; }

        /// <summary>
        /// Get or create-and-cache the default <see cref="IRepository{T}"/> for an entity type T.
        /// </summary>
        /// <typeparam name="T">
        ...
        /// </remarks>
        public Contracts.IRepository<T> GetRepositoryForEntityType<T>() where T : class
        {
            return GetRepository<IRepository<T>>(
                    _repositoriesFactories.GetRepositoryFactoryForEntityType<T>()
                );
        }
        ...
        /// caller wants to specify the specific factory to use rather than one of the injected
        /// <see cref="RepositoriesFactories"/>.
        /// </param>
        /// <remarks>
        /// Looks for the requested repository in its cahce, returning if found.
        ...
        /// <summary>
        /// Set the repository for the type T that this provider should return.
        /// </summary>
        /// <remarks>
        /// Plug in a custom repository if you don't want this provider to create one.
        /// Usefull in testing and when developing without a backend
        ...
        /// </summary>
        /// <typeparam name="T">
        /// Type of the repository to make.
        /// </typeparam>
        /// <param name="factory">
        /// Factory with <see cref="DbContext"/> arugment. Used to make the repository.
        /// If null, gets factory from <see cref="_repositoryFactories"/>.
        /// </param>
        /// <param name="dbContext">
        /// The <see cref="DbContext"/> with wich to initialize the repository.
        /// </param>
        protected virtual T MakeRepository<T>(Func<DbContext, object> factory, DbContext dbContext)
        {
            var f = factory ?? _repositoriesFactories.GetRepositoryFactory<T>();
            if (f == null)
            {
                throw new NotImplementedException("No factory for repository type, " + typeof(T).Name);
            }
