...
{
    public class CustomerLogsManager : ICustomerLogsManager
    {
        public IUow Uow { get; set; }

        public FilteredResults<CustomerLog> GetFiltered(FilterArgumentsCustomerLogs parameters)
        {
            // set defaults
            parameters.Length = parameters.Length == 0 ? 10 : parameters.Length;
            parameters.Filter = parameters.Filter ?? new List<Filter>();

            var query = Uow.CustomerLogs.GetAll().Where(cl => cl.CustomerId == parameters.CustomerId);
            ...
            return new FilteredResults<CustomerLog>()
            {
                Data = query.ToList(),
