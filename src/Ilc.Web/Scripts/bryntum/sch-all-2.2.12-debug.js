...
Ext.define("Sch.feature.AbstractTimeSpan", {
    ...
    /**
     * Return element class for data record.
     * 
...
/*
 * It seems that this class is used only in example externaldragdrop.
 */
...
Ext.define("Sch.feature.PointDragZone", {
    ...
    startDrag : function() {
        ...
        this.dragData.refElements = this.proxy.el.select('.sch-event');
...
Ext.define("Sch.feature.SchedulerDragZone", {
    ...
    getCopyKeyPressed : function() {
        return this.enableCopy && this.dragData.eventObj[this.enableCopyKey.toLowerCase() + 'Key'];
    },
    ...
    startDrag : function() {
        ...
        if(this.copyKeyPressed) {
            this.dragData.refElements.addCls('sch-event-copy');
            this.dragData.originalHidden = true;
        }
...
Ext.define('Sch.mixin.Lockable', {
    ...
    patchSyncHorizontalScroll : function(grid) {
        ...
        function syncHorizontalScroll(left, setBody) {
            var target = this.getScrollTarget().el;

            if (target) {
                this.syncHorizontalScroll.call(this, target.dom.scrollLeft, setBody);
            }
        }

        // Override scrollTask
        grid.scrollTask = new Ext.util.DelayedTask(syncHorizontalScroll, grid);
    },
...
Ext.define("Sch.data.TimeAxis", {
    ...
    */    
    ...
    constructor : function(config) {
        ...
        me.on(Ext.versions.touch ? 'refresh' : 'datachanged', function() { me.fireEvent('reconfigure', me); });
...
Ext.define("Sch.plugin.Zones", {
    ...
    getElementData : function(viewStart, viewEnd, records, isPrint) {
        var schedulerView   = this.schedulerView,
            data            = [],
            record, spanStart, spanEnd, region, zoneData, width, templateData;
        ...
        for (var i = 0, l = records.length; i < l; i++) {
            ...
            if (spanStart && spanEnd && Sch.util.Date.intersectSpans(spanStart, spanEnd, viewStart, viewEnd)) {
                region  = schedulerView.getTimeSpanRegion(Sch.util.Date.max(spanStart, viewStart), Sch.util.Date.min(spanEnd, viewEnd), this.expandToFitView);
...
Ext.define('Sch.mixin.Zoomable', {
    ...
    zoomLevels: [
        ...
        //MINUTE
        { width: 30,    increment: 30,  resolution: 5, preset: 'minuteAndHour' },
        { width: 60,    increment: 30,  resolution: 5, preset: 'minuteAndHour' },
        { width: 130,   increment: 30,  resolution: 5, preset: 'minuteAndHour' },
    ...
    */
    switchViewPreset: function (preset, startDate, endDate, initial) {
        ...
        if (this.fireEvent('beforeviewchange', this, preset, startDate, endDate) !== false) {
            ...
            this.applyViewSettings(preset, initial); 
    ...
    applyViewSettings: function (preset, initial) {
        ...
        if (this.orientation === 'horizontal') {
            view.setRowHeight(initial ? this.rowHeight || preset.rowHeight : preset.rowHeight || this.rowHeight, true);
        }
    },
...
Ext.define('Sch.mixin.TimelinePanel', {
    ...
    */
    ...
    configureColumns : function() {
        ...
        // Clone it to make sure we handle the case of array object on the class prototype
        var columns         = this.columns = (this.columns || []).slice();
    ...
    inheritables : function() {
        return {
            ...
            initComponent: function () {
                ...
                // (it is disabled by default in our Lockable mixin, because its slow)
            ...
            applyViewSettings: function (preset, initial) {
                ...
                if (this.orientation === 'horizontal') {
                    height = this.rowHeight = initial ? this.rowHeight || preset.rowHeight : preset.rowHeight || this.rowHeight;
                } else {
    ...
    */
    ...
    setOrientation: function (orientation, force) {
        ...
        if (orientation === 'horizontal') {
            ...
            me.reconfigure(me.timeAxis, me.verticalColumns.concat(me.createResourceColumns()));
