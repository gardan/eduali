...
Ext.define('Sch.util.Date', {
     ...
     * @param {Date} start Start date
     * @param {Date} end End date
     * @return {Float} true number of days between the two dates
     * 
     * @static
     */
...
Ext.define("Sch.preset.ViewPreset", {
    ...
    */
    ...
    /**
     * @cfg {String} columnLinesFor Defines header level column lines will be drawn for. See {@link Sch.mixin.AbstractTimelinePanel#columnLines}
     */
...
if (!Ext.ClassManager.get("Sch.feature.AbstractTimeSpan")) {
    ...
    Ext.define("Sch.feature.AbstractTimeSpan", {
        ...
        /**
         * @private
         * @cfg {Ext.XTemplate} template Template used to render the header elements
         */
        headerTemplate : null,
...
if (!Ext.ClassManager.get("Sch.view.model.TimeAxis")) {
    ...
    Ext.define("Sch.view.model.TimeAxis", {
        ...
        // default header level to draw coulmn lines for
        columnLinesFor : 'middle',
...
This plugin enables zooming to the timespan, selected with drag&drop in the header area of the scheduler panel.

Zooming will be performed to the nearest zooming level that will make all columns to fit in the scheduling view width,
additionally a column width of that zooming level will be slightly adjusted to make fitting better.

After zooming, selected span will appear centered in the scheduling view. Overall rendered timespan will be extended
in the same way as regular {@link Sch.panel.SchedulerGrid#zoomToLevel} method does, to allow scrolling. 

**NOTICE*: This plugin supports only horizontal-oriented scheduler.
...
Ext.define("Sch.plugin.HeaderZoom", {
    ...
    init : function (scheduler) {
        ...
        this.scheduler      = scheduler;
        this.timeAxisColumn = scheduler.down('timeaxiscolumn');

        this.timeAxisColumn.on({
    ...
    onTimeAxisColumnRender : function (s) {
        this.proxy = this.timeAxisColumn.el.createChild({ cls : 'sch-drag-selector' });
        this.initEl(this.timeAxisColumn.el);
    },
    ...
    onEnd : function (e) {
        if (this.proxy) {
            ...
            var viewModel   = scheduler.getSchedulingView().timeAxisViewModel;
            
            var region      = this.getRegion().constrainTo(this.headerRegion);
            var range       = scheduler.getSchedulingView().getStartEndDatesFromRegion(region);
            
            scheduler.zoomToSpan({
                start   : timeAxis.floorDate(range.start, false, viewModel.getBottomHeader().unit, 1),
                end     : timeAxis.ceilDate(range.end, false, viewModel.getBottomHeader().unit, 1)
            });
        }
    },
...
Ext.define("Sch.feature.ColumnLines", {
    ...
    getData : function() {
        ...
        if (panel.isHorizontal()) {
            this.timeAxisViewModel.forEachInterval(this.timeAxisViewModel.columnLinesFor, function(start, end, i) {
                if (i > 0) {
...
Ext.define('Sch.view.TimelineGridView', {
    ...
    calculateInfiniteScrollingDateRange : function (srollLeftDate, unit, increment, tickWidth) {
        ...
        return {
            start   : DATE.add(srollLeftDate, unit, -bufferedTicks * increment),
            end     : DATE.add(srollLeftDate, unit, Math.ceil((viewWidth / tickWidth + bufferedTicks) * increment))
        };
    },
...
Ext.define('Sch.mixin.Zoomable', {
     ...
     * @return {Int} Return number of milliseconds per pixel.
     */
    ...
    */
     ...
     * @param {Int} options.customWidth Lowest tick width. Might be increased automatically
...
//                weekStartDay        : this.timeAxis.weekStartDay
//            });
...
//                timeAxis.consumeViewPreset(preset);
            ...
            return this.zoomToLevel(levelToZoom, 
                this.calculateOptimalDateRange(centerDate, availableWidth, zoomLevel),
                { 
...
Ext.define('Sch.mixin.AbstractTimelinePanel', {
    ...
    * @param {Date} date The date to which to scroll the time line
    * @param {Boolean optional} animate Whether or not scroll should be animated 
    */
